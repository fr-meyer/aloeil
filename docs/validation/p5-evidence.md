...
5. Save an encrypted archive to a user-chosen location. Recreate the Activity during the save, confirm the write completes, and inspect the resulting file; discard any file left incomplete by a force-stop. On a fresh app profile, restore with the passphrase and compare every synthetic reading, sitting, revision, note and deletion marker. Try a wrong passphrase and corrupt file; neither may change existing phone data. Keep file and passphrase separately.
